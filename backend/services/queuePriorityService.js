...
async function changeBrandPriority(queueType, brandName, newPosition) {
  try {
    ...
    const queueKey =
      queueType === "pending" ? QUEUES.PENDING_BRANDS : QUEUES.FAILED_BRANDS;
    const queueName = queueType === "pending" ? "pending" : "failed";

    logger.info(
      `Changing priority for brand "${brandName}" in ${queueName} queue to position ${newPosition}`
    );
    ...
    if (targetIndex >= queueLength) {
      throw new Error(
        `Position ${newPosition} is out of range. Queue has ${queueLength} items`
      );
    }
    ...
    for (let i = 0; i < currentItems.length; i++) {
      try {
        ...
        const brandId =
          brandData.id || brandData.brand_id || brandData.queue_id;
        ...
        if (brandId) {
          try {
            ...
            logger.error(
              `Error fetching brand ${brandId} from database:`,
              dbError
            );
          }
        }
    ...
    if (currentIndex === -1) {
      throw new Error(
        `Brand with name "${brandName}" not found in ${queueName} queue`
      );
    }
    ...
    logger.info(
      `Successfully moved brand "${brandName}" from position ${
        currentIndex + 1
      } to position ${newPosition} in ${queueName} queue`
    );

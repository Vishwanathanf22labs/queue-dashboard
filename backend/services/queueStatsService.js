...
async function getBrandsScrapedStats(date = null) {
  try {
    ...
    const brandsStats = {
      ...
      total_brands: parseInt(allStats.brands_scraped || 0) + parseInt(allStats.brands_scrapped_failed || 0),
      success_rate: allStats.brands_scraped && allStats.brands_scrapped_failed 
        ? Math.round((parseInt(allStats.brands_scraped) / (parseInt(allStats.brands_scraped) + parseInt(allStats.brands_scrapped_failed))) * 100)
        : 0
    };

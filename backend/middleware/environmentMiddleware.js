...
function environmentMiddleware(req, res, next) {
  ...
  if (!validEnvironments.includes(environment)) {
    logger.warn(`Invalid environment received: ${environment}, defaulting to production`);
    req.environment = 'production';
  } else {
    req.environment = environment;
  }
  
  logger.debug(`Request using environment: ${req.environment} (URL: ${req.url}, Method: ${req.method})`);

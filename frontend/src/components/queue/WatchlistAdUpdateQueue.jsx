...
const WatchlistAdUpdateQueue = ({ watchlistAdUpdateQueue, loading, error, onPageChange, onSortChange, onSearch, searchTerm, onClearSearch, isSearching }) => {
  const [searchParams, setSearchParams] = useSearchParams();
  
  // Get pagination and sorting state from URL params
  const currentPage = parseInt(searchParams.get('watchlistAdUpdatePage')) || 1;
  ...
  useEffect(() => {
    if (watchlistAdUpdateQueue) {
      const isSearchActive = searchTerm && searchTerm.trim().length > 0;
      
    // Only update original totals when NOT searching
    if (!isSearchActive) {
      setOriginalTotals({
        total_items: watchlistAdUpdateQueue.pagination?.total_items || 0,
        total_ads: watchlistAdUpdateQueue.total_ads_watchlist || 0
      });
    }
    }
  }, [watchlistAdUpdateQueue, searchTerm]);
  ...
  if (loading && !watchlistAdUpdateQueue) {
    return (
      <Card>
        <LoadingSpinner />
      </Card>
  ...
  if (error) {
    return (
      <Card>
        <ErrorDisplay title="Error Loading Watchlist Ad-Update Queue" message={error} />
      </Card>
    );
  }
  ...
  const watchlistBrands = watchlistAdUpdateQueue.brands || [];
  const totalPages = watchlistAdUpdateQueue.pagination?.total_pages || 1;
  const totalItems = watchlistAdUpdateQueue.pagination?.total_items || 0;
  const itemsPerPage = watchlistAdUpdateQueue.pagination?.per_page || 10;
  const apiCurrentPage = watchlistAdUpdateQueue.pagination?.current_page || 1;
  ...
  const handlePageChange = (newPage) => {
    ...
    const tableSection = document.getElementById('watchlist-ad-update-table-section');
    if (tableSection) {
  ...
  const handleSortChange = (field, order) => {
    ...
    newParams.set('watchlistAdUpdateSortBy', field);
    newParams.set('watchlistAdUpdateSortOrder', order);
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      <div>
        <h2 className="text-lg sm:text-xl lg:text-2xl font-bold text-blue-900">Watchlist Ad-Update Count</h2>
      </div>
          ...
          <div className="flex items-center">
            ...
            <div className="ml-2 sm:ml-3">
              ...
              <p className="text-lg sm:text-xl lg:text-2xl font-bold text-purple-900">
                {originalTotals.total_ads || watchlistAdUpdateQueue?.total_ads_watchlist || 0}
              </p>
            </div>
          </div>
        </Card>
          ...
          <div className="flex items-center">
            ...
            <div className="ml-2 sm:ml-3">
              ...
              <p className="text-lg sm:text-xl lg:text-2xl font-bold text-green-900">
                {watchlistAdUpdateQueue?.analytics?.current_page_total_ads || 0}
              </p>
            </div>
          </div>
        </Card>
        ...
        <div className="flex flex-col space-y-3 sm:flex-row sm:items-center sm:justify-between sm:space-y-0 sm:gap-4">
          <div className="flex-1 max-w-md">
            <SearchInput
              value={searchTerm || ''}
              onChange={onSearch}
              placeholder="Search watchlist ad-update by name, ID, or page ID..."
          ...
          <div className="flex items-center space-x-3 sm:space-x-4 text-xs sm:text-sm text-gray-600">
            ...
            <span>Showing: {watchlistBrands.length}</span>
          </div>
        </div>
      </Card>

      <Card id="watchlist-ad-update-table-section">
        <div className="flex flex-col space-y-3 sm:space-y-0 sm:flex-row sm:items-center sm:justify-between mb-3 sm:mb-4">
          <div>
            <h3 className="text-base sm:text-lg font-semibold text-blue-900">Watchlist Ad-Update in Queue</h3>
            <div className="flex flex-wrap items-center gap-3 mt-1">
              ...
              <div className="flex items-center space-x-1">
                ...
                <span className="text-sm font-bold text-gray-900">
                  {watchlistAdUpdateQueue?.analytics?.pre_computed_counters?.active || 0}
                </span>
              </div>
              ...
              <div className="flex items-center space-x-1">
                ...
                <span className="text-sm font-bold text-gray-900">
                  {watchlistAdUpdateQueue?.analytics?.pre_computed_counters?.waiting || 0}
                </span>
              </div>
              ...
              <div className="flex items-center space-x-1">
                ...
                <span className="text-sm font-bold text-gray-900">
                  {watchlistAdUpdateQueue?.analytics?.pre_computed_counters?.delayed || 0}
                </span>
              </div>
              ...
              <div className="flex items-center space-x-1">
                ...
                <span className="text-sm font-bold text-gray-900">
                  {watchlistAdUpdateQueue?.analytics?.pre_computed_counters?.completed || 0}
                </span>
              </div>
              ...
              <div className="flex items-center space-x-1">
                ...
                <span className="text-sm font-bold text-gray-900">
                  {watchlistAdUpdateQueue?.analytics?.pre_computed_counters?.failed || 0}
                </span>
              </div>
              ...
              <div className="flex items-center space-x-1">
                ...
                <span className="text-sm font-bold text-gray-900">
                  {watchlistAdUpdateQueue?.analytics?.pre_computed_counters?.prioritized || 0}
                </span>
              </div>
            </div>
          </div>
          <div className="flex flex-col space-y-2 sm:space-y-0 sm:flex-row sm:items-center sm:space-x-2">
            <div className="text-xs sm:text-sm text-gray-600">
              Showing {startIndex + 1} to {endIndex} of {totalItems} brands
            </div>
          </div>
        </div>
        ...
        {isSearching ? (
            ...
            <div className="flex flex-col items-center space-y-4">
              ...
              <div className="text-center">
                ...
                <p className="text-sm text-gray-500">
                  Searching for "{searchTerm}" in watchlist ad-update queue
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="text-center py-6 sm:py-8">
            ...
            <h3 className="text-base sm:text-lg font-medium text-gray-900 mb-2">
              {searchTerm ? 'No watchlist brands found' : 'No watchlist brands in queue'}
            </h3>
            <p className="text-sm sm:text-base text-gray-500">
              {searchTerm ? 'Try adjusting your search terms' : 'All watchlist ad-updates processed'}
            </p>
          </div>
        ) : null}
...
export default WatchlistAdUpdateQueue;

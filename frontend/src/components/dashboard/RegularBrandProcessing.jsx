...
import { Play, Clock, Hourglass, XCircle, CheckCircle, ExternalLink } from 'lucide-react';
import { openFacebookAdLibrary } from '../../utils/facebookAdLibrary';

const RegularBrandProcessing = ({ allBrandProcessingData, loading, error }) => {
  if (loading) {
    return (
      <Card className="p-6">
  ...
  if (!allBrandProcessingData) {
    return (
      <Card className="p-6">
        <h3 className="text-base sm:text-lg font-semibold text-gray-900 mb-3 sm:mb-4">Regular Brand Processing</h3>
        <div className="text-center py-6 sm:py-8">
          <Play className="h-8 w-8 sm:h-12 sm:w-12 text-gray-300 mx-auto mb-2 sm:mb-3" />
          <p className="text-gray-500 text-sm sm:text-base">No brand processing data available</p>
        </div>
      </Card>
    );
  }
      ...
      <Card key={status}>
        ...
        {jobs.length > 0 ? (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <span className="text-sm font-medium text-gray-600">
                {jobs.length} job{jobs.length !== 1 ? 's' : ''} {status}
              </span>
          ...
          <div className="text-center py-6 sm:py-8">
            ...
            <p className="text-gray-500 text-sm sm:text-base">No {status} jobs</p>
          </div>
        )}
      </Card>
    );
    ...
    <div className="grid gap-4 sm:gap-6 grid-cols-1">
      <Card>
        ...
        {allJobs.length > 0 ? (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 max-h-96 overflow-y-auto scrollbar-hide">
              {allJobs.map((job, index) => (
                <div key={`${job.job_id}-${index}`} className={`flex flex-col space-y-2 p-3 rounded-lg border min-h-[200px] ${job.job_status === 'active' ? 'bg-green-50 border-green-200' :
                  ...
                  <div className="flex items-center space-x-3 mb-2">
                    <div className={`w-6 h-6 sm:w-8 sm:h-8 rounded-full flex items-center justify-center flex-shrink-0 ${job.job_status === 'active' ? 'bg-green-100' :
          ...
          <div className="text-center py-6 sm:py-8">
            <Play className="h-8 w-8 sm:h-12 sm:w-12 text-gray-300 mx-auto mb-2 sm:mb-3" />
            <p className="text-gray-500 text-sm sm:text-base">No regular brand processing jobs</p>
          </div>
        )}
      </Card>
    </div>
...
export default RegularBrandProcessing;